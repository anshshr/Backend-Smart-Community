    ...
    async getByFilter(req, res) {
        try {
            const data = z.object({
                ...
                status: z.enum(ProductStatus, "Available values are  AVAILABLE  REQUESTED SOLD "),
            });
    ...
    async updatePurchaseProduct(req, res) {
        try {
            const requetstObject = z.object({
                UpdatedStatus: z.enum(RequestStatus, "Available values are PENDING , ACCEPTED , REJECTED  "),
    ...
    async purchaseProduct(req, res) {
        try {
            const purchaseObject = z.object({
                ...
                paymentStatus: z.enum(PaymentStatus, "Available status are PENDING , PAID , FAILED"),
            });
